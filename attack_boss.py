...
def auto_update_and_attack():
    ...
    # Thiết lập device đầu tiên làm device mặc định
    set_device(devices[0]['device_id'])
    ...
    while True:  # Vòng lặp chính để cập nhật boss
        try:
            # Bước 1: Cập nhật vị trí boss
            print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Đang cập nhật vị trí boss...")
