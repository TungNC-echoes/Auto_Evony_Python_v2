...
from utils.adb_utils import adb_command, input_text
...
def attack_boss(boss_name, x_coord, y_coord, image_folder, troops_count, start_time=None):
    ...
    try:
        ...
        while location_retry_count < max_retries:
            if find_and_click_button("attack/location", 'none', 0.7):
                ...
                if location_retry_count >= max_retries:
                    print("Đã thử tìm nút location tối đa số lần, thực hiện ESC và thử lại...")
                    # Thực hiện ESC
                    if adb_command('adb shell input keyevent KEYCODE_ESCAPE'):
                        print("Đã nhấn ESC thành công, thử lại tìm location...")
                        # Reset lại số lần thử
                        location_retry_count = 0
                        time.sleep(2)
                        continue
                    else:
                        print("Không thể thực hiện thao tác ESC")
                        return False
                
                time.sleep(2)
                
        if location_retry_count >= max_retries:
            print("Đã thử tìm nút location tối đa số lần, không thể tiếp tục")
            return False
            
        time.sleep(2)


        find_and_click_right_edge("attack/x")
        time.sleep(2)

        # Xóa tọa độ cũ và nhập tọa độ X
        ...
        find_and_click_right_edge("attack/y")
        time.sleep(2)

        # Xóa tọa độ cũ và nhập tọa độ Y
        adb_command('adb shell input keyevent KEYCODE_MOVE_END')
        for _ in range(5):
            adb_command('adb shell input keyevent KEYCODE_DEL')
        input_text(y_coord)
        ...
        if os.path.exists(image_folder):
            for image_file in os.listdir(image_folder):
                if image_file.endswith(('.jpg', '.JPG', '.png', '.PNG')):
                    ...
                    if find_and_click_button(f"attack/{boss_name}/{image_name}", 'none', 1, 3, 0.7):
...
def execute_attack_sequence(start_time=None, troops_count="300000"):
    ...
    try:
        ...
        while retry_count < max_retries:
            ...
            if not find_and_click_button("attack/5minutes"):
                print("Không thể click vào nút 5minutes")
                return False
            time.sleep(2)

            # Kiểm tra nút doi_quan_san_co
            if check_button_exists("doi_quan_san_co", 'none', 0.9):
                    ...
                    if not find_and_click_button("hanh_quan", 'none', 1, 1, 0.7):
                        ...
                        if check_button_exists("xac_nhan"):
                            ...
                            if handle_insufficient_stamina():
                                ...
                                if find_and_click_button("hanh_quan", 'none', 1, 1, 0.7):
                                    return True
                            else:
                ...
                print(f"Lần {retry_count + 1}: Không thấy nút doi_quan_san_co, đợi 1 phút và thử lại...")
